...
// rAF coalesces bursts into one update per paint, but an occluded window
// suspends its rAF queue entirely — a notebook the user cannot currently see
// would never reach the servers. The timer backstop keeps the sync moving;
...
class NotebookLspBridge {
  ...
  revealCell(cellId, range) {
    ...
    return editor.revealCell?.(index, position);
  }
