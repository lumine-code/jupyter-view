...
class JupyterNotebookEditor {
  ...
  _initWithDocument(notebookDocument) {
    ...
    atom.packages.triggerActivationHook?.("jupyter.adapter:item-used");
  ...
  triggerSourceEditorActivationHooks() {
    ...
    atom.packages.triggerActivationHook?.("jupyter-view:grammar-used");
  }
  ...
  registerSourceEditorForServices() {
    ...
    const registration = atom.textEditors.add(this.sourceEditor, { role: "background" });
  ...
  async promptExportPath(options) {
    const { filePath } = await atom.window.showSaveDialog(options);
    return filePath;
  }
