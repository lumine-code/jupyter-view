...
class JupyterNotebookEditor {
  static deserialize(state, deserializeServices = null) {
    return new JupyterNotebookEditor(null, state, deserializeServices);
  }
    ...
    this._loadingPromise = (async () => {
      try {
        ...
        const doc = state.documentId
          ? await documentRegistry.getOrCreateDocumentById(state.documentId)
          : null;

        if (doc) {
          this.activeCellIndex = Math.max(
            0,
            Math.min(state.viewState?.activeCellIndex || 0, Math.max(0, doc.getCellCount() - 1)),
          );
          this._initWithDocument(doc);
          await this._sourceEditorSetupPromise;
          this.restorePaneViewState(state.viewState);
        } else {
  ...
  _initWithDocument(notebookDocument) {
    ...
    this._buildView(this._containerElement.ownerDocument);
    ...
    this.emitter.emit("did-change-file-state", this.document.getFileState());

    // Notify services (e.g. navigation-panel) that headers are now available
    this.emitter.emit("did-change");
    this.emitter.emit("did-change-navigation");

    // A restored item exists before its document does; consumers tracking the
    // item (the language-server bridge) hear here that the document arrived.
    this.emitter.emit("did-attach-document", notebookDocument);

    // Apply pending state from copy() if any (cursor positions, mode, scroll)
    this._applyPendingStateSoon();
  }

  _buildView(domDocument) {
  ...
  /**
   * The document attached to this item. Fires once for a deserialized item,
   * whose document loads asynchronously after construction; an item built
   * with its document never fires it.
   */
  onDidAttachDocument(callback) {
    return this.emitter.on("did-attach-document", callback);
  }
  ...
  subscribeToDocument() {
    this.disposables.add(
      this.document.onDidChange((event) => {
        ...
        this.emitter.emit("did-change");
        if (event?.category === "history") this.emitter.emit("did-change-navigation");
      }),
      ...
      this.document.onDidInsertCells(({ index, cells }) => {
        for (let offset = 0; offset < cells.length; offset++) {
          if (index + offset <= this.activeCellIndex) this.activeCellIndex++;
          this._adjustSelectionForInsert(index + offset);
        }
      }),
      ...
      this.document.onDidDeleteCells(({ indices }) => {
        for (const index of indices) {
          if (index < this.activeCellIndex) this.activeCellIndex--;
          else if (index === this.activeCellIndex) {
            this.activeCellIndex = Math.min(
              this.activeCellIndex,
              Math.max(0, this.document.getCellCount() - 1),
            );
          }
          this._adjustSelectionForDelete(index);
        }
      ...
      this.document.onDidMoveCells(({ indices, targetIndex }) => {
        this.activeCellIndex = this._mapIndexForMultiMove(
          this.activeCellIndex,
          indices,
          targetIndex,
        );
        if (this.view?.selectedCells?.size) {
          this.view.selectedCells = new Set(
            Array.from(this.view.selectedCells).map((index) =>
              this._mapIndexForMultiMove(index, indices, targetIndex),
            ),
          );
        }
      }),
      ...
      this.document.onDidChangeFileState((fileState) => {
        ...
        if (fileState !== FileState.UNMODIFIED) {
    ...
    this.disposables.add(
      lumine.workspace.onDidChangeActivePaneItem((item) => {
        if (item === this && this.view) {
          this.getDOMWindow().requestAnimationFrame(() => {
            if (
              this.view._mouseButtonDown ||
              this.view.element.contains(this._containerElement.ownerDocument.activeElement)
            ) {
              return;
            }
  ...
  updateView(event = null) {
    if (this.view) {
      if (
        event &&
        !event.structural &&
        event.cellIds?.length > 0 &&
        event.reason !== "notebook-metadata"
      ) {
        if (event.category === "runtime" || event.category === "transient") {
          this.scheduleCellViewUpdates(event.cellIds);
          return;
        }
        this.view.updateCells(event.cellIds);
        return;
      }
      this.view.update({
        editor: this,
      ...
      this._emitActiveTextEditorsChange();
    }
  }

  scheduleCellViewUpdates(cellIds) {
    this._pendingCellViewUpdates = this._pendingCellViewUpdates || new Set();
    for (const cellId of cellIds) this._pendingCellViewUpdates.add(cellId);
    if (this._cellViewUpdateFrame) return;
    this._cellViewUpdateFrame = this.getDOMWindow().requestAnimationFrame(() => {
      this._cellViewUpdateFrame = null;
      const pending = Array.from(this._pendingCellViewUpdates);
      this._pendingCellViewUpdates.clear();
      if (!this._destroyed) this.view?.updateCells?.(pending);
    });
  }
  ...
  async _setupSourceEditor() {
    this.sourceController = this.document.getSourceController(this._serializedSourceEditorState);
    await this.sourceController.attach(this, this._serializedSourceEditorState);
    this.sourceEditor = this.sourceController.sourceEditor;
    this._serializedSourceEditorState = null;
    // The file editor exists now; whoever resolves editors through the item
    // protocol saw null while the setup was still pending.
    this._emitActiveTextEditorsChange();
  }

  updateSourceEditorFromNotebook(reason = "document-change") {
    this.sourceController?.scheduleSnapshot(reason, this);
  }

  commitSourceEditorSnapshot(reason = "sync", options = {}) {
    return this.sourceController?.commitSnapshot(reason, this, options);
  }
  ...
  captureHistoryUIState(options = {}) {
    ...
    const cursorCells = options.allCursors ? cells : activeCell ? [activeCell] : [];
  ...
  capturePaneViewState() {
    return {
      ...this.captureHistoryUIState({ allCursors: true }),
      scrollTop: this.view?.cellsContainer?.scrollTop || 0,
    };
  }

  restorePaneViewState(viewState) {
    if (!viewState || !this.view) return;
    this.activeCellIndex = this.resolveHistoryActiveCellIndex(viewState);
    this.updateView();
    this.restoreHistoryUIState(viewState, { restoreSelection: true });
    this.getDOMWindow().requestAnimationFrame(() => {
      if (this.view?.cellsContainer) this.view.cellsContainer.scrollTop = viewState.scrollTop || 0;
    });
  }
  ...
  getCellEditorById(cellId) {
    return this.view?.cellViews?.get(cellId)?.editor || null;
  }

  /**
   * Reports a cell's TextEditor being created or destroyed — cells change
   * type, markdown flips between rendered and editable, and etch builds
   * editors asynchronously after structural changes. `editor` is null when
   * the cell's editor went away.
   */
  onDidChangeCellEditors(callback) {
    return this.emitter.on("did-change-cell-editors", callback);
  }

  notifyCellEditorChange(cellId, editor) {
    if (this._destroyed) return;
    this.emitter.emit("did-change-cell-editors", { cellId, editor });
    this._emitActiveTextEditorsChange();
  }
  ...
  /**
   * Text-editor resolution protocol, read by the workspace (see
   * Workspace#getActiveFileTextEditor and #getActiveEmbeddedTextEditor): the
   * backing source editor carries the notebook file's identity — encoding and
   * line endings describe the .ipynb — while the active cell's editor is what
   * is being edited, so the grammar tile names the cell's language.
   */
  getFileTextEditor() {
    return this.sourceEditor || null;
  }
  ...
  getActiveEmbeddedTextEditor() {
    const cell = this.document?.cells?.[this.activeCellIndex];
    if (!cell) return null;
    return this.view?.cellViews?.get(cell.id)?.editor || null;
  }

  onDidChangeActiveTextEditors(callback) {
    return this.emitter.on("did-change-active-text-editors", callback);
  }

  // Fired liberally — on view refreshes, cell editor churn and source-editor
  // setup; the workspace dedupes on the resolved editors' identity.
  _emitActiveTextEditorsChange() {
    if (this._destroyed) return;
    this.emitter.emit("did-change-active-text-editors");
  }
  ...
  ownsLinterMessage(message) {
    ...
    if (!notebookPath || typeof location.file !== "string") return false;
    // Providers spell the same path differently — separators, drive-letter
    // case — and the rest of the linter stack compares normalized paths too.
    if (process.platform === "win32") {
      const fold = (value) => value.replaceAll("/", "\\").toLowerCase();
      return fold(location.file) === fold(notebookPath);
    }
    return location.file === notebookPath;
  }
  ...
  revealLinterMessage(message) {
    const index = this.getLinterMessageCellIndex(message);
    if (index < 0) return;
    return this.revealCell(index, message.location.position.start);
  }

  /**
   * Activate this notebook, scroll a cell into view, and place the cursor.
   * The generic reveal behind linter navigation and language-server locations;
   * `position` is cell-relative. A cell with no editor — a rendered markdown
   * cell — is still activated and scrolled to.
   */
  async revealCell(index, position = { row: 0, column: 0 }) {
    if (index < 0 || index >= (this.document?.getCellCount?.() ?? 0)) return;

    const opened = await lumine.workspace.open(this, { searchAllPanes: true });
    if (!opened) return;
    ...
    this.getDOMWindow().requestAnimationFrame(() => {
      ...
      this.getDOMWindow().requestAnimationFrame(() => {
        const editor = this.getCellEditor(index + 1);
        if (!editor) return;
  ...
  async revealNavigationHeader(header) {
  ...
  observeNavigationHeaders(callback) {
    ...
    const emit = (options) => {
      ...
      frame = this.getDOMWindow().requestAnimationFrame(() => {
        frame = null;
        if (!disposed) {
    ...
    return new Disposable(() => {
      disposed = true;
      if (frame !== null) {
        this.getDOMWindow().cancelAnimationFrame(frame);
      }
  ...
  getDOMWindow() {
    return this._containerElement.ownerDocument.defaultView;
  }

  beginWindowSurfaceTransition() {
    if (!this.view || !this.document) return null;
    this.sourceController?.flushPendingChanges?.(this);
    const viewState = this.capturePaneViewState();
    this.view.destroy();
    this.view = null;
    this._containerElement.replaceChildren();

    const rebuild = (surface) => {
      if (this._destroyed || this.view) return;
      this._buildView(surface.document);
      this.restorePaneViewState(viewState);
      this._emitActiveTextEditorsChange();
    };
    return {
      commit: ({ to }) => rebuild(to),
      rollback: ({ from }) => rebuild(from),
    };
  }

  getFileState() {
    return this.document?.getFileState() || FileState.UNMODIFIED;
  }
  ...
  shouldPromptToSave(options = {}) {
    const fileState = this.getFileState();
    if (fileState === FileState.UNMODIFIED) return false;
    if (!lumine.config.get("core.promptOnCloseDirtyBuffer")) return false;
    if (
      options.windowCloseRequested &&
      options.projectHasPaths &&
      lumine.stateStore.isConnected()
    ) {
      return fileState === FileState.CONFLICTED || fileState === FileState.REMOVED;
    }
  ...
  serialize() {
    ...
    return {
      ...
      viewState: this.capturePaneViewState(),
    };
  }
  ...
  prepareForNotebookOperation() {
    if (this.sourceController?.applying) return;
    this.flushPendingCellSourceChanges();
  ...
  /**
   * Record a cell's own language, picked through the grammar selector.
   * null returns the cell to the notebook's language.
   */
  setCellLanguage(index, languageId) {
    if (!this.document) return;
    const cell = this.document.getCell(index);
    if (!cell || cell.getLanguage?.() === (languageId || null)) return;
    this.prepareForNotebookOperation();
    this.document.setCellLanguage(index, languageId);
  }
  ...
  _insertCellsFromData(startIndex, cellsData) {
    this.document.insertCellsFromData(startIndex, cellsData, this);
  ...
  performSourceEditorHistory(direction) {
    ...
    return this.sourceController?.performHistory(direction, this);
  }
  ...
  mergeCellBelow() {
    ...
    // Update first cell
    this.document.updateCellSource(this.activeCellIndex, mergedSource, this);
  ...
  toggleCellInput() {
    ...
    this.document.toggleCellInput(this.activeCellIndex);
  ...
  updateCellSource(index, source) {
    ...
    this.document.updateCellSource(index, source, this);
  }
  ...
  flushPendingCellSourceChanges() {
    ...
    for (const [cellId, cellView] of this.view.cellViews) {
      if (!cellView || !cellView.editor || !cellView._editorIsDirty) continue;
  ...
  async promptExportPath(options) {
    const { filePath } = await lumine.workspace.showSaveDialogForPaneItem(this, options);
    return filePath;
  }
  ...
  _mapIndexForMultiMove(index, movedIndices, targetIndex) {
    const sorted = [...movedIndices].sort((a, b) => a - b);
    const movedPosition = sorted.indexOf(index);
    if (movedPosition !== -1) return targetIndex + movedPosition;

    const compactIndex = index - sorted.filter((movedIndex) => movedIndex < index).length;
    return compactIndex >= targetIndex ? compactIndex + sorted.length : compactIndex;
  }
  ...
  destroy() {
    ...
    this.sourceController?.detach(this);
    if (this._cellViewUpdateFrame) {
      this.getDOMWindow().cancelAnimationFrame(this._cellViewUpdateFrame);
      this._cellViewUpdateFrame = null;
    }
