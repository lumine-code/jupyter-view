...
class JupyterNotebookEditor {
  ...
  _initWithDocument(notebookDocument) {
    ...
    lumine.hooks.trigger?.("jupyter.adapter:item-used");
  ...
  scheduleCellViewUpdates(cellIds) {
    ...
    this._cellViewUpdateFrame = requestAnimationFrame(() => {
  ...
  classifyHistoryChange(notebook) {
    ...
    const metadataChanged = !metadataEqual(this.document?.metadata || {}, notebook?.metadata || {});
    let structural = beforeCells.length !== afterCells.length;
    const changedCellIds = [];
    const changedCellMetadataIds = [];
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (!metadataEqual(before.metadata, after.metadata)) {
        changedCellMetadataIds.push(after.id);
      }
    }
    ...
    return {
      ...
      structural,
      changedCellIds,
      changedCellMetadataIds,
      metadataChanged,
      sourceOnly:
        !structural &&
        !metadataChanged &&
        changedCellMetadataIds.length === 0 &&
        changedCellIds.length > 0,
    };
  }
  ...
  captureHistoryUIState(options = {}) {
    ...
    return {
      ...
      cursorByCellId,
      lineEndings: Array.from(this.document?.getLineEndings?.() || ["\n"]),
      firstLineEnding: this.document?.firstLineEnding || "\n",
    };
  }
  ...
  destroy() {
    ...
    if (this._cellViewUpdateFrame) {
      cancelAnimationFrame(this._cellViewUpdateFrame);
      this._cellViewUpdateFrame = null;
    }
