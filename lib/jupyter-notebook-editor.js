...
class JupyterNotebookEditor {
  ...
  registerSourceEditorForServices() {
    ...
    // so config applies, but excluded from cross-editor features like
    // completion sourcing (its buffer holds metadata and base64 outputs).
    ...
    // Only pane items are linted on their own, so the source editor is handed
    // to the linter explicitly; its diagnostics reach the cells through the
    // `linter.adapter` this package provides.
    this.document._sourceEditorDisposable.add(
      require("./linter-editors").addLinterEditor(this.sourceEditor),
    );
  }

  scheduleSourceEditorSync() {
    return this.scheduleSourceEditorSnapshot("sync");
  }

  scheduleSourceEditorSnapshot(reason = "sync") {
    return this.sourceController?.scheduleSnapshot(reason, this);
  }
  ...
  applySourceEditorSnapshot(reason = "source-editor-change") {
    if (this.sourceController) {
      return this.sourceController.applySnapshot(reason, this);
    }
