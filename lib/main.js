...
function notebookForEvent(context, event) {
  ...
  if (container) {
    ...
    return notebook && !notebook._destroyed ? notebook : null;
  }
...
module.exports = {
  provideBackgroundTips() {
    return {
      packageName: "jupyter-view",
      tips: [
        "You can start a Jupyter notebook from scratch with {{ 'jupyter-view:new-notebook' | keystroke }}",
        "In a notebook you can turn the selected cell into Markdown with {{ 'jupyter-view:change-cell-to-markdown' | keystroke }}",
        "Language servers reach inside notebooks: completions, hover and diagnostics work in cells when an ide package serves the cell language.",
      ],
    };
  },
  ...
  initialize(state = {}) {
    ...
    // Service providers/consumers are connected synchronously by the package
    // manager. Resolve their implementation modules during initialization so
    // a cold `activateProvidedServices` call only constructs the already
    // cached lightweight facades (and keeps the activation stopwatch focused
    // on registration rather than module graph parsing).
    getJupyterAdapterService();
    getNotebookSearchAdapter();
    getNotebookScrollmap();
    require("./lsp-bridge");
    require("./output-renderer");
  },
  ...
  activate(state = {}) {
    ...
    // Cell editor elements stay mounted in command mode, but they are not the
    // active embedded editor then. Stop the workspace grammar picker at the
    // mode boundary so a context or scripted dispatch cannot edit a dormant
    // cell grammar behind the command-mode UI.
    this.disposables.add(
      lumine.commands.add(".jupyter-notebook.command-mode", {
        "grammar-selector:show": (event) => event.stopPropagation(),
      }),
    );
    ...
    // Restored editors are already tracked by their deserializer. Sweep the
    // remaining pane items after the activation critical path; the next frame
    // repeats the check for pane items that finish attaching during restore.
    queueMicrotask(() => {
      if (this.disposables) this.discoverNotebookEditors();
    });
