...
class NotebookDocument {
  ...
  async save() {
    ...
    try {
      ...
      const historyStateIdAtSave = this.currentHistoryStateId;
      const savedRuntimeRevision = this.runtimeRevision;
      const lineEnding = this.getPreferredLineEnding();
      const normalizesLineEndings =
        this.lineEndings.size !== 1 || !this.lineEndings.has(lineEnding);
      const text = formatLineEndings(JSON.stringify(content, null, 2), lineEnding);
      await fsp.writeFile(this.filePath, text, "utf8");
      let savedHistoryStateId = historyStateIdAtSave;
      if (normalizesLineEndings) {
        this._setLineEndingState(new Set([lineEnding]), lineEnding);
        if (this.currentHistoryStateId === historyStateIdAtSave) {
          this.currentHistoryStateId = uuidv4();
