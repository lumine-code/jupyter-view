...
function normalizeLanguage(language) {
  ...
  normalized = LANGUAGE_ALIASES[normalized] || normalized;

  // Jupyter kernels commonly put the runtime version in either the language
  // or kernelspec name. Keep those names on the language id that Lumine's
  // grammar registry understands.
  if (/^python(?:[\s._-]?\d)/.test(normalized)) return "python";
  if (/^julia(?:[\s._-]?\d)/.test(normalized)) return "julia";
  if (/^(?:r|irkernel)(?:[\s._-]?\d)/.test(normalized)) return "r";
  if (/^(?:c\+\+|cpp)(?:[\s._-]?\d)/.test(normalized)) return "cpp";
...
function getNotebookLanguage(metadata = {}) {
  ...
  return inferLanguageFromKernelName(metadata.kernelspec) || "python";
}

function inferLanguageFromKernelName(kernelSpec = {}) {
  const names = [kernelSpec?.name, kernelSpec?.display_name];
  ...
  for (const value of names) {
    ...
    if (/(?:^|[^a-z])python(?:[^a-z]|$)|^py(?:thon)?\d/.test(name)) return "python";
    if (/^ir(?:kernel)?(?:[^a-z]|$)|(?:^|[^a-z])r(?:[^a-z]|$)/.test(name)) return "r";
    if (/(?:^|[^a-z])julia(?:[^a-z]|$)/.test(name)) return "julia";
    if (/c\+\+|(?:^|[^a-z])cpp(?:[^a-z]|$)|xeus[-_]?cling|^xcpp/.test(name)) return "cpp";
    if (/(?:^|[^a-z])javascript(?:[^a-z]|$)|(?:^|[^a-z])node(?:\.js)?(?:[^a-z]|$)/.test(name)) {
      return "javascript";
    }
    if (/(?:^|[^a-z])typescript(?:[^a-z]|$)/.test(name)) return "typescript";
    if (/(?:^|[^a-z])ruby(?:[^a-z]|$)|^iruby(?:[^a-z]|$)/.test(name)) return "ruby";
    if (/(?:^|[^a-z])scala(?:[^a-z]|$)/.test(name)) return "scala";
    if (/(?:^|[^a-z])java(?:[^a-z]|$)/.test(name)) return "java";
    if (/(?:^|[^a-z])rust(?:[^a-z]|$)/.test(name)) return "rust";
    if (/(?:^|[^a-z])go(?:lang)?(?:[^a-z]|$)/.test(name)) return "go";
  }
